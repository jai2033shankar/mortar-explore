...
class Mortar::Local::Controller
  def explore(data_directory, port, recsys=nil)
    port ||= 3000 
    explorer = Mortar::Local::Explorer.new()
    # Startup Web server
    ...
    begin
      ...
      if e.message == "no acceptor (port is in use or requires root privileges)"
        display "\nPort #{port} may be in use.  Please run with '-t PORT' and select a different port.\n"
      else
        display e
      end
    end
